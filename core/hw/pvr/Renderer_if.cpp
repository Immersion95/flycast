...
#ifdef LIBRETRO
		// In Libretro, call renderFramebuffer() synchronously right after Render()
		// instead of waiting for vblank. This eliminates the extra frame of lag
		// introduced by the async pvrQueue, matching standalone behavior.
		if (config::EmulateFramebuffer && !taContext->rend.isRTT)
		{
